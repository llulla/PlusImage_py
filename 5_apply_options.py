...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일 선택하세요.", filetypes=(("모든 파일","*.*"),("PNG파일", "*.png")),initialdir=r"D:\picture")
...
def browse_sest_path():
    ...
    if folder_selected == "": #사용자 취소누른 경우
        return
...
def merge_image():
    ...
    try:
        ...
        # print("max widths : ",max_widths, "total heigth : ",total_heigts)
        ...
        for idx, img in enumerate(images):
            ...
            progress = (idx +1) / len(images) * 100 # 실제 퍼센트 정보
            p_var.set(progress)
            progress_bar.update()

        #포멧 옵션 처리
        file_name = "Plus_Image."+ img_format
